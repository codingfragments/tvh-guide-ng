...
class RecordingManager {
  ...
  async scheduleRecording(eventId) {
    try {
      const response = await fetch(`${this.apiUrl}/api/dvr/entry/create_by_event`, {
        method: 'POST',
        headers: {
  ...
  async cancelRecording(dvrUuid) {
    const response = await fetch(`${this.apiUrl}/api/dvr/entry/cancel`, {
      method: 'POST',
      headers: {
        Authorization: `Basic ${this.auth}`,
        'Content-Type': 'application/json',
  ...
  async isAlreadyScheduled(eventId) {
    ...
    const response = await fetch(`${this.apiUrl}/api/dvr/entry/grid?status=scheduled&limit=0`, {
...
// Usage
const recordingManager = new RecordingManager('http://localhost:9981', 'api-user', 'password');
...
async function createSeriesRecording(seriesTitle, channelUuid = null) {
  ...
  const response = await fetch(`${apiUrl}/api/dvr/autorec/create`, {
    method: 'POST',
    headers: {
...
async function scheduleWithPadding(eventId, prePadding = 2, postPadding = 5) {
  ...
  const eventResponse = await fetch(`${apiUrl}/api/epg/events/load`, {
    method: 'POST',
    headers: {
      Authorization: `Basic ${auth}`,
      'Content-Type': 'application/json',
  ...
  const response = await fetch(`${apiUrl}/api/dvr/entry/create`, {
    method: 'POST',
    headers: {
      Authorization: `Basic ${auth}`,
      'Content-Type': 'application/json',
...
const moviesResponse = await fetch(`${apiUrl}/api/epg/events/grid?filter=${encodeURIComponent(moviesFilter)}&limit=0`);
